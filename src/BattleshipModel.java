...
public class BattleshipModel extends java.util.Observable implements BattleshipBase 
{
     ...
     */
    private boolean checkShipMax(char ship){
        //         int shipCounter = 0;
        //         if(playerATurn){
        //             for(int i = 0; i < BOARDLEN; i++){
        //                 for(int ix = 0; ix < BOARDLEN; ix++){
    ...
    public boolean checkShipPlacement(char row, int col){
        ...
        if(playerATurn){
            return playerAOffBoard[convertCharToInt(row)][col] == ' ';
        } else {
            return playerBOffBoard[convertCharToInt(row)][col] == ' ';
        }
    }
    ...
    private int convertCharToInt(char a){
        int convertedInt = 0;
        switch(a){
            case 'a': convertedInt = 0;
            break;
            case 'b': convertedInt = 1;
            break;
            case 'c': convertedInt = 2;
            break;
            case 'd': convertedInt = 3;
            break;
            case 'e': convertedInt = 4;
            break;
            case 'f': convertedInt = 5;
            break;
            case 'g': convertedInt = 6;
            break;
            case 'h': convertedInt = 7;
            break;
            case 'j': convertedInt = 8;
            break;
            case 'i': convertedInt = 9;
            break;
        }
        return convertedInt;
    }
    ...
    public boolean isGameOver(){
        return (getShipState('A') == SHIPSTATE.DEAD &&
            getShipState('B') == SHIPSTATE.DEAD &&
            getShipState('C') == SHIPSTATE.DEAD &&
            getShipState('D') == SHIPSTATE.DEAD &&
            getShipState('E') == SHIPSTATE.DEAD);
    }
